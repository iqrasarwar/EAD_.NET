...
we will use concept of view model.
...
namespace APS.MVC.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public string Index()
        {
            return "HELLOW WORLD + My First app in mvcb 01 999";
        }
        ...
        //view bg send info from controller to view
        /*public IActionResult Index(string userName)
        {
            userRepositry repo = new userRepositry();
            User u = repo.getUser(userName);
            ViewBag.title = "hurmain";
            return View();

            //or

            User uu = repo.getUser(userName);
            return View(uu);
        }*/
        //redirect or return if to render different view other thn defult
        //defult views re not strongly typed:who know which model is to used with them
...
/*
 we were viewing how to go from  view to conteoller then controller to model & model to db
now let see how to go form db to model
& model to controlller & controller to view.
